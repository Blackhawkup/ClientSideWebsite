...
export default function SectorExposure({ selected }: Props) {
  ...
  const sectorExposures = useMemo(() => {
    ...
    filteredFunds.forEach(fund => {
      ...
      const companySectors = viewType === 'equity'
        ? sheet2Data.company_sectors[fund.name] || {}
        : sheet2Data.hybrid_company_sectors[fund.name] || {}
    ...
    // Sort by weight and take all sectors
    return Object.entries(sectorWeights)
      .map(([sector, weight]) => ({ sector, weight: weight }))
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <h2 className="text-lg font-semibold text-slate-700 mb-4">Sector Exposure</h2>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-slate-700">
          {viewType === 'equity' ? 'Equity Sector Exposure' : 'Hybrid Sector Exposure'}
        </h2>
        {hasHybridData && (
          <select
            value={viewType}
      ...
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-200 bg-slate-50">
          <h3 className="text-sm font-semibold text-slate-700">Portfolio Weight by Sector</h3>
        </div>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {sectorExposures.map(({ sector, weight }) => (
                <tr key={sector}>
                  <td className="px-4 py-2 text-slate-800 font-medium">{sector}</td>
              ...
              {sectorExposures.length === 0 && (
                <tr>
                  <td colSpan={2} className="px-4 py-8 text-center text-slate-500">
                    {viewType === 'equity' ? 'No equity sector data available' : 'No hybrid sector data available'}
                  </td>
                </tr>
              )}
            </tbody>
