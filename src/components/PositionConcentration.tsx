...
export default function PositionConcentration({ selected }: Props) {
  ...
  const positionConcentration = useMemo(() => {
    if (!sheet2Data) return { top3: 0, top5: 0 }
    ...
    const sortedWeights = Object.values(companyWeights)
      .sort((a, b) => b - a)

    const top3 = sortedWeights.slice(0, 3).reduce((sum, w) => sum + w, 0)
    const top5 = sortedWeights.slice(0, 5).reduce((sum, w) => sum + w, 0)
    const top10 = sortedWeights.slice(0, 10).reduce((sum, w) => sum + w, 0)
    const top20 = sortedWeights.slice(0, 20).reduce((sum, w) => sum + w, 0)

    return { top3, top5, top10, top20 }
  }, [sheet2Data, equityFunds, allocationMap])

  if (loading) {
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <h2 className="text-lg font-semibold text-slate-700 mb-4">Position Concentration</h2>
  ...
  const concentrations = [
    { label: 'Top 3', value: positionConcentration.top3 },
    { label: 'Top 5', value: positionConcentration.top5 },
    { label: 'Top 10', value: positionConcentration.top10 },
    { label: 'Top 20', value: positionConcentration.top20 }
  ]
